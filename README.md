...
Уточнение по работе с апи по пункту play with bot

Чтобы мне сыграть с ботом, мне надо отправить запрос на /joinRoom с юзером Bot
для того чтобы бот ходил я отправляю запрос на  /get-bot-step, если при этом текущий ход бота(об этом узнаю по запросу /get-step-order)
 и после того как получил шаг, от имени бота отправляю его на do-step